...
public class ApplicationsController {
	...
	@ResponseBody
	public List<String> dependencies1(@RequestParam String application, @RequestParam(required = false) boolean reverse) {
		List<String> rows = new ArrayList<String>();

		Set<String> indirectly = new HashSet<String>();
		appendDependency1(rows, reverse, application, 0, new HashSet<String>(), indirectly);
		return rows;
	}

	private void appendDependency1(List<String> rows, boolean reverse, String application, int level, Set<String> appended, Set<String> indirectly) {
		...
		if (level > 5) {
			...
			buf.append("level " + level + ":" + application);
			if (appended.contains(application)) {
				buf.append("ERROR!!! Cycle");
				end = true;
			}
		}
		...
		if (dependencies != null && dependencies.size() > 0) {
			for (String dependency : dependencies) {
				appendDependency1(rows, reverse, dependency, level + 1, appended, indirectly);
			}
		}
	...
	private void appendDependency(List<String> rows, boolean reverse, String application, int level, Set<String> appended, Set<String> indirectly) {
		...
		if (level > 0) {
			...
			buf.append(reverse ? "<i class=\"fa fa-level-down\" style=\"-webkit-transform: rotate(180deg);margin-right:5px;\"></i> "
					: "<i class=\"fa fa-level-up\" style=\"-webkit-transform: rotate(90deg);margin-right:5px;\"></i> ");
		}
		boolean end = false;
		if (level > 5) {
